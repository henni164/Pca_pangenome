...
scoring_data <- read.csv("Supplemental_figure8_pathotypes.csv", header=TRUE, check.names=FALSE, stringsAsFactors=FALSE)
...
phylo_tree <- read.tree(file = "Supplemental_figure8_aus_pruned.nwk")
...
dev.copy(pdf, "Supplemental_figure8_tanglegram.pdf", width = 8, height = 8)
dev.off()
